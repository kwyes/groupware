...
if($mode == "search") {
	...
	/*
	$query = "SELECT head.ID AS ID, head.Type AS Type, head.Seq AS Seq, stat.StatusDesc AS Status, comp.companyDesc AS CompanyID, mem.memName AS UserName, convert(char(10), head.SubmitDate, 126) AS SubmitDate, head.Subject AS Subject, head.RegDate AS RegDate ".
			 "FROM E_DOC_Header AS head ".
			 "INNER JOIN Member AS mem ON mem.memID = head.UserID ".
			 "INNER JOIN Company AS comp ON head.CompanyID = comp.companyID ".
			 "INNER JOIN ApprovalStatus AS stat ON head.Status = stat.StatusID ".
			 "WHERE head.UserID = '$UserID' AND convert(char(10), head.SubmitDate, 126) <= '$today' AND head.Status = 2 ".
			 "ORDER BY head.RegDate DESC";
*/
	...
	<table width="100%">
		...
		<tr>
			<td height="40">
				<table width="100%">
					<tr>
						<td width="360" align="left" class="content_title">올린 결재 문서함 > 결재완료</td>
						<td align="right" style="padding: 14px;">&nbsp;</td> 
					</tr>
				</table>
			</td>
		</tr>
		<!-- offer_submit_list TITLE END -->

		<!-- offer_submit_list MAIN START -->
		<tr>
			<td height="1" valign="top" style="border-top: 1px #c9c9c9 solid"></td>
		</tr>

		<!-- offer_submit_list SEARCH BOX START -->
		<form name="form_search" action="?page=e_doc&menu=offer&sub=submit" enctype="multipart/form-data" method="post" accept-charset="utf-8">
...
?>
							<tr height="25">
								<td class="docid bb"><a href="javascript:post_to_url('?page=e_doc&menu=offer&sub=view_submit', {'ID':'<?=$row['ID']?>', 'Seq':'<?=$row['Seq']?>','Type':'<?=$row['Type']?>'});"><?=$ID; ?></a></td>
								<td class="content bb"><?=get_docName($row['Type']); ?></td>
								<td class="content bb"><a href="javascript:post_to_url('?page=e_doc&menu=offer&sub=view_submit', {'ID':'<?=$row['ID']?>', 'Seq':'<?=$row['Seq']?>','Type':'<?=$row['Type']?>'});"><?=Br_iconv($row['Subject']); ?></a></td>
...
<?					} ?>
		...
		<!-- offer_submit_list PAGE NAVIGATION START -->
		<tr>
			<td align="center">
<?
...
?>
		...
		<!-- offer_submit_list PAGE NAVIGATION END -->
