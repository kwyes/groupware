...
<input type="hidden" name="Subject" value="<?=Br_iconv($row['Subject']);?>">
	...
	<table width="100%">
		...
		<tr>
			<td height="40">
				<table width="100%">
					<tr>
						<td width="360" align="left" class="content_title">받은 결재 문서함 > <?=Br_iconv(get_ApprovalStatus($row['Status']))?></td>
						...
						<td align="center" valign="top">
							<table width="100%" style="border: 1px solid #c9c9c9; table-layout:fixed;">
								...
								<tr class="doc_border">
									...
									<td  align="left" class="doc_field_content"><?=get_company_name($row['CompanyID'])?></td>
								</tr>
								<tr class="doc_border">
									<?	if($row['Status'] == 1) {
											$font_color = "#0000FF";
...
<?										} ?>
								...
								<tr>
									...
									<td height="350" colspan="4" style="border: 1px solid #c9c9c9; padding: 10px 12px;"><?=str_replace('-ms-word-break:','word-break:' , str_replace('\"', '"', Br_iconv($row['Contents']))); ?></td>
								</tr>
								<tr class="doc_border">
									<td height="30" align="center" class="doc_field_name"><b>결재자 의견</b></td>
									<td class="doc_field_content" colspan="3" style="padding-top:5px; padding-bottom:5px;">
...
function printPage(Id, Type, Seq) {
	var popUrl = "print_preview_voucher.php?ID="+Id+"&Type="+Type+"&Seq="+Seq;
	var popOption = "width=900, height=600, toolbar=0, location=0, directories=0, resizable=1, menubar=0, scrollbars=yes, status=no";
