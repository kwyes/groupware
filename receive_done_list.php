...
if($mode == "search") {
	...
	if($folderNum) {
		...
		$query = "SELECT head.ID AS ID, head.Type AS Type, head.Seq AS Seq, head.Status AS Status, head.CompanyID AS CompanyID, mem.memID AS UserID, CONVERT(char(10), head.SubmitDate, 126) AS SubmitDate, head.Subject AS Subject, head.RegDate AS RegDate ".
				 ...
				 "INNER JOIN ApprovalList AS app ON app.DocID = head.ID AND app.DocType = head.Type AND app.DocSeq = head.Seq AND (app.ApprovalStatus = 1 OR app.ApprovalStatus = 5) AND app.ApprovalUserID = '$UserID' ".
				 ...
				 "WHERE (CONVERT(char(10), head.SubmitDate, 126) BETWEEN '$dateStart' AND '$dateEnd') AND (head.Status = $Status) AND (head.Type LIKE '%$docType%') AND (head.Subject LIKE '%$docSubject%') AND (head.CompanyID LIKE '%$docCompany%') ".
		...
		$query = "SELECT head.ID AS ID, head.Type AS Type, head.Seq AS Seq, head.Status AS Status, head.CompanyID AS CompanyID, mem.memID AS UserID, CONVERT(char(10), head.SubmitDate, 126) AS SubmitDate, head.Subject AS Subject, head.RegDate AS RegDate ".
				 ...
				 "INNER JOIN Member AS mem ON mem.memName LIKE '%$submitUser%' AND head.UserID = mem.memID ".
				 "INNER JOIN ApprovalList AS app ON app.DocID = head.ID AND app.DocType = head.Type AND app.DocSeq = head.Seq AND (app.ApprovalStatus = 1 OR app.ApprovalStatus = 5) AND app.ApprovalUserID = '$UserID' ".
				 "WHERE (CONVERT(char(10), head.SubmitDate, 126) BETWEEN '$dateStart' AND '$dateEnd') AND (head.Status = $Status OR head.Status = 2 OR head.Status = 5) AND (head.Type LIKE '%$docType%') AND (head.Subject LIKE '%$docSubject%') AND (head.CompanyID LIKE '%$docCompany%') ".
				 "ORDER BY head.RegDate DESC";
	}
	...
	*/
	...
	if($folderNum) {
		...
		$query = "SELECT head.ID AS ID, head.Type AS Type, head.Seq AS Seq, head.Status AS Status, head.CompanyID AS CompanyID, head.UserID AS UserID, CONVERT(char(10), head.SubmitDate, 126) AS SubmitDate, head.Subject AS Subject, head.RegDate AS RegDate ".
				 "FROM E_DOC_Header AS head ".
				 "INNER JOIN ApprovalList AS app ON app.DocID = head.ID AND app.DocType = head.Type AND app.DocSeq = head.Seq AND (app.ApprovalStatus = $Status OR app.ApprovalStatus = 5) AND app.ApprovalUserID = '$UserID' ".
				 "WHERE convert(char(10), head.SubmitDate, 126) <= '$today' AND (head.Status = $Status OR head.Status = 2 OR head.Status = 5) ".
				 "ORDER BY head.RegDate DESC";
	}
}
...
<td width="" align="left" valign="top">
	<table width="100%">
		...
		<form name="form_search" action="<?=($folderNum) ? '?page=e_doc&menu=receive&sub=receive_folder' : '?page=e_doc&menu=receive&sub=done' ?>" enctype="multipart/form-data" method="post" accept-charset="utf-8">
		<input type="hidden" name="mode" value="search">
		<input type="hidden" name="list" value=1>
<?		if($folderNum) { ?>
			<input type="hidden" name="showReceive" value=<?=$showReceive; ?>>
...
<?					} else {?>
	...
	<?					while($row = mssql_fetch_array($query_result)) { ?>
							...
							<tr height="25">
								<td class="docid bb">
<?									if($folderNum) { ?>
										<a href="javascript:post_to_url('?page=e_doc&menu=receive&sub=view_folder&showReceive=<?=$showReceive; ?>&folderNum=<?=$folderNum; ?>', {'ID':<?=$row['ID']?>, 'Seq':<?=$row['Seq']?>, 'Type':<?=$row['Type']?>});"><?=create_DocID($row['ID'], $row['Seq']); ?></a>
<?									} else { ?>
										<a href="javascript:post_to_url('?page=e_doc&menu=receive&sub=view_done', {'ID':<?=$row['ID']?>, 'Seq':<?=$row['Seq']?>, 'Type':<?=$row['Type']?>});"><?=create_DocID($row['ID'], $row['Seq']); ?></a>
<?									} ?>
								</td>
...
<?								if($row['Status'] == 1) {
								...
								<td align="left" class="content bb">
<?									if($folderNum) { ?>
										<a href="javascript:post_to_url('?page=e_doc&menu=receive&sub=view_folder&showReceive=<?=$showReceive; ?>&folderNum=<?=$folderNum; ?>', {'ID':<?=$row['ID']?>, 'Seq':<?=$row['Seq']?>, 'Type':<?=$row['Type']?>});"><?=Br_iconv($row['Subject']); ?></a>
<?									} else { ?>
										<a href="javascript:post_to_url('?page=e_doc&menu=receive&sub=view_done', {'ID':<?=$row['ID']?>, 'Seq':<?=$row['Seq']?>, 'Type':<?=$row['Type']?>});"><?=Br_iconv($row['Subject']); ?></a>
<?									} ?>									
								</td>
