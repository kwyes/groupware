...
if($mode == "search") {
	...
	$query = "SELECT head.ID AS ID, head.Type AS Type, head.Seq AS Seq, head.Status AS Status, head.CompanyID AS CompanyID, mem.memID AS UserID, CONVERT(char(10), head.SubmitDate, 126) AS SubmitDate, head.Subject AS Subject, head.RegDate AS RegDate ".
			 ...
			 "WHERE (CONVERT(char(10), head.SubmitDate, 126) BETWEEN '$dateStart' AND '$dateEnd') AND (head.Type LIKE '%$docType%') AND (head.Subject LIKE '%$docSubject%') AND (head.CompanyID LIKE '%$docCompany%') AND head.Status = 1 ".
	...
	*/
	...
	$query = "SELECT ID, Type, Seq, Status, CompanyID, UserID, CONVERT(char(10), SubmitDate, 126) AS SubmitDate, Subject, RegDate ".
			 "FROM E_DOC_Header ".
			 "WHERE (CONVERT(char(10), SubmitDate, 126) BETWEEN '$dateStart' AND '$dateEnd') AND (Status = 1 OR Status = 2 OR Status = 5 OR Status = 6) AND Type = 9 ".
			 "ORDER BY RegDate DESC";

} else {
	$query = "SELECT ID, Type, Seq, Status, CompanyID, UserID, CONVERT(char(10), SubmitDate, 126) AS SubmitDate, Subject, RegDate ".
			 "FROM E_DOC_Header ".
			 "WHERE convert(char(10), SubmitDate, 126) <= '$today' AND (Status = 1 OR Status = 2 OR Status = 5 OR Status = 6) AND Type = 9 ".
			 "ORDER BY RegDate DESC";
}
	...
	<table width="100%">
		...
		<tr>
			<td height="40">
				<table width="100%">
					<tr>
						<td width="360" align="left" class="content_title">Item Spot Check 리스트</td>
		...
		<form name="form_search" action="?page=e_doc&menu=itemSpotCheck&sub=itemSpotCheckList" method="post" accept-charset="utf-8">
...
<?								$query = "SELECT docID, docName FROM DocKind WHERE docID = 9 ORDER BY docID";
...
<?								} ?>
						...
						<td width="400" style="padding: 3px 0 0 10px; border-right: 0;">
					...
					<tr height="30">
						<td colspan="5" align="right" style="padding: 4px 5px 0 0;"><button>검색</button></td>
					</tr>
				</table>				
			</td>
		</tr>
...
<?						while($row = mssql_fetch_array($query_result)) { ?>
							<tr height="25">
<?								if($row['Status'] == 6) { ?>
									<td class="docid bb"><a href="javascript:post_to_url('?page=e_doc&menu=itemSpotCheck&sub=itemSpotCheckCheck', {'ID':'<?=$row['ID']?>', 'Seq':'<?=$row['Seq']?>','Type':'<?=$row['Type']?>'});"><?=create_DocID($row['ID'], $row['Seq']); ?></a></td>
<?								} else if($row['Status'] == 2 || $row['Status'] == 1 || $row['Status'] == 5) { ?>
									<td class="docid bb"><a href="javascript:post_to_url('?page=e_doc&menu=itemSpotCheck&sub=itemSpotCheckView', {'ID':'<?=$row['ID']?>', 'Seq':'<?=$row['Seq']?>','Type':'<?=$row['Type']?>'});"><?=create_DocID($row['ID'], $row['Seq']); ?></a></td>
<?								} ?>
<?								if($row['Status'] == 1) {
									$font_color = "#0000FF";
									$doc_status = "결제완료";
...
<?								if($row['Status'] == 6) { ?>
									<td class="content bb"><a href="javascript:post_to_url('?page=e_doc&menu=itemSpotCheck&sub=itemSpotCheckCheck', {'ID':'<?=$row['ID']?>', 'Seq':'<?=$row['Seq']?>','Type':'<?=$row['Type']?>'});"><?=Br_iconv($row['Subject']); ?></a></td>
<?								} else if($row['Status'] == 2 || $row['Status'] == 1 || $row['Status'] == 5) { ?>
									<td class="content bb"><a href="javascript:post_to_url('?page=e_doc&menu=itemSpotCheck&sub=itemSpotCheckView', {'ID':'<?=$row['ID']?>', 'Seq':'<?=$row['Seq']?>','Type':'<?=$row['Type']?>'});"><?=Br_iconv($row['Subject']); ?></a></td>
<?								} ?>
								<td class="content bb" style="color:<?=$font_color; ?>"><?=$doc_status; ?></td>
