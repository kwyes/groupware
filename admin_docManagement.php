...
if($mode == "search") {
	...
	*/
	...
	$query = "SELECT head.ID AS ID, head.Type AS Type, head.Seq AS Seq, head.Status AS Status, head.CompanyID AS CompanyID, mem.memID AS UserID, CONVERT(char(10), head.SubmitDate, 126) AS SubmitDate, head.Subject AS Subject, head.RegDate AS RegDate ".
			 "FROM E_DOC_Header AS head ".
			 "INNER JOIN Member AS mem ON mem.memName LIKE '%$submitUser%' AND head.UserID = mem.memID ".
			 "WHERE (CONVERT(char(10), head.SubmitDate, 126) BETWEEN '$dateStart' AND '$dateEnd') AND (head.Type LIKE '%$docType%') AND (head.Subject LIKE '%$docSubject%') AND (head.CompanyID LIKE '%$docCompany%') AND Status = 5 ".
			 "ORDER BY head.RegDate DESC";

} else {
	$query = "SELECT ID, Type, Seq, Status, CompanyID, UserID, CONVERT(char(10), SubmitDate, 126) AS SubmitDate, Subject, RegDate ".
			 "FROM E_DOC_Header ".
			 "WHERE convert(char(10), SubmitDate, 126) <= '$today' AND Status = 5 ".
...
<td width="" align="left" valign="top">
	<table width="100%">
		<!-- admin_docManagement TITLE START -->
		<tr>
			<td height="40">
				<table width="100%">
					<tr>
						<td width="360" align="left" class="content_title">반려문서 관리함</td>
		...
		<!-- admin_docManagement SEARCH BOX START -->
		<form name="form_search" action="?page=admin&menu=docManagement" enctype="multipart/form-data" method="post" accept-charset="utf-8">
...
<?								} ?>
		...
		<!-- admin_docManagement SEARCH BOX END -->

		<!-- admin_docManagement LIST START -->
		<tr>
			<td>
...
<?						while($row = mssql_fetch_array($query_result)) { ?>
							<tr height="25">
								<td class="docid bb"><a href="javascript:post_to_url('?page=admin&menu=docManagement_view', {'ID':'<?=$row['ID']?>', 'Seq':'<?=$row['Seq']?>','Type':'<?=$row['Type']?>'});"><?=create_DocID($row['ID'], $row['Seq']); ?></a></td>
<?								if($row['Status'] == 1) {
									$font_color = "#0000FF";
								...
								<td class="content bb"><a href="javascript:post_to_url('?page=admin&menu=docManagement_view', {'ID':'<?=$row['ID']?>', 'Seq':'<?=$row['Seq']?>','Type':'<?=$row['Type']?>'});"><?=Br_iconv($row['Subject']); ?></a></td>
...
<?					} ?>
		...
		<!-- admin_docManagement PAGE NAVIGATION START -->
		<tr>
			<td align="center">
<?
...
?>
		...
		<!-- admin_docManagement PAGE NAVIGATION END -->

		<tr>
			<td height="30"></td>
		</tr>
		<!-- admin_docManagement LIST END -->
	</table>
</td>
<!-- admin_docManagement END -->
				</tr>
			</table>
		</td>	
	</tr>
