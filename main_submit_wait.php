<?
	...
	$s_where = " UserID = '$UserID' AND Status = 2 ";
			...
			<table width="100%" class="doc_main_table" style="border-top:#c9c9c9 1px solid;">
				...
				<? } else { ?>
					<? while($ListRow = mssql_fetch_array($ListSel)) { ?>
						<tr height="25">
							<td width="10%" class="docid bb"><a href="javascript:post_to_url('index.php?page=e_doc&menu=offer&sub=view_submit', {'ID':'<?=$ListRow['ID']?>', 'Seq':'<?=$ListRow['Seq']?>','Type':'<?=$ListRow['Type']?>'});"><?=create_DocID($ListRow['ID'], $ListRow['Seq'])?></td>
							<td width="10%" class="content bb"><?=get_docName($ListRow['Type'])?></td>
							<td align="left" class="content bb"><a href="javascript:post_to_url('index.php?page=e_doc&menu=offer&sub=view_submit', {'ID':'<?=$ListRow['ID']?>', 'Seq':'<?=$ListRow['Seq']?>','Type':'<?=$ListRow['Type']?>'});"><?=Br_iconv($ListRow['Subject'])?></a></td>
							<td width="10%" class="content bb" style="color:#088A08;"><?=get_doc_approval($ListRow['Status'])?></td>
