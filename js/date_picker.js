...
function datePicker(e, elm) {
	...
	currentElement = elm;		// added
	temp = arguments[2];		// added
...
function fBuildHtml() {
	...
	var calHtml = "<table border='0' style='background-color:#C0C0C0;'>";
...
function fDrawCal(iYear, iMonth) {
	...
	var calHtml = "<tr>";
	for(i=0; i<7; i++) calHtml += "<td width='20' valign='middle' align='center' style='font:bold 10px Tahoma; color:black; background-color:yellow'>" + WeekDay[i] + "</td>";
	calHtml += "</tr>";
	var i = 0;
	for (w = 1; w < 7; w++) {
		calHtml += "<tr>";
...
function fSetSelected(aCell){
	...
	// added
	if(temp) {
		schedule_calculate_day(temp, currentElement);
	}
}

// added
function PrevNext_month(mode) {
	var select_month = document.getElementsByName('select_month')[0].value;
	select_month = select_month.split("-");
	var sYear = parseInt(select_month[0]);
	var sMonth = parseInt(select_month[1]);
	...
	document.getElementsByName('select_month')[0].value = sYear+"-"+(parseInt(sMonth+100).toString().substring(1,3));
}