...
$UserCID = $_SESSION['memberCID'];
$UserDID = $_SESSION['memberDID'];
$today = date("Y-m-d");
$Status = 1;
...
if($mode == "search") {
	...
	$query = "SELECT head.ID AS ID, head.Type AS Type, head.Seq AS Seq, head.Status AS Status, head.CompanyID AS CompanyID, mem.memID AS UserID, CONVERT(char(10), head.SubmitDate, 126) AS SubmitDate, head.Subject AS Subject, head.RegDate AS RegDate ".
			 ...
			 "INNER JOIN Cooperation AS coop ON head.ID = coop.DocID AND head.Type = coop.DocType AND head.seq = coop.DocSeq AND coop.CompanyID = $UserCID AND coop.DeptID = $UserDID AND coop.CoopList > 1 ".
	...
	$query = "SELECT head.ID AS ID, head.Type AS Type, head.Seq AS Seq, head.Status AS Status, head.CompanyID AS CompanyID, head.UserID AS UserID, CONVERT(char(10), head.SubmitDate, 126) AS SubmitDate, head.Subject AS Subject, head.RegDate AS RegDate ".
			 "FROM E_DOC_Header AS head ".
			 "INNER JOIN Cooperation AS coop ON head.ID = coop.DocID AND head.Type = coop.DocType AND head.seq = coop.DocSeq AND coop.CompanyID = $UserCID AND coop.DeptID = $UserDID AND coop.CoopList > 1 ".
			 "WHERE convert(char(10), head.SubmitDate, 126) <= '$today' AND head.Status = $Status AND head.Type = 2 ".
			 "ORDER BY head.RegDate DESC";
}
...
<!-- receive_wait_list START -->
<td width="" align="left" valign="top">
	<table width="100%">
		<!-- receive_wait_list TITLE START -->
		<tr>
			<td height="40">
				<table width="100%">
					<tr>
						<td width="360" align="left" class="content_title">부서 수신함</td>
						<td align="right" style="padding: 14px;">&nbsp;</td> 
					</tr>
				</table>
			</td>
		</tr>
		<!-- receive_wait_list TITLE END -->

		<!-- receive_wait_list MAIN START -->
		<tr>
			<td height="1" valign="top" style="border-top: 1px #c9c9c9 solid"></td>
		</tr>

		<!-- receive_wait_list SEARCH BOX START -->
		<form name="form_search" action="?page=e_doc&menu=dept_receive" enctype="multipart/form-data" method="post" accept-charset="utf-8">
...
<?								} ?>
		...
		<!-- receive_wait_list SEARCH BOX END -->

		<!-- receive_wait_list LIST START -->
		<tr>
			<td>
...
<?							$is_read = check_is_read($row['Type'], $row['ID'], $row['Seq'], $UserID); ?>
							<tr height="25">
								<td class="docid bb"><a href="javascript:post_to_url('?page=e_doc&menu=dept_receive&sub=view_dept', {'ID':<?=$row['ID']?>, 'Seq':<?=$row['Seq']?>, 'Type':<?=$row['Type']?>});"><?=create_DocID($row['ID'], $row['Seq']); ?></a></td>
								<td class="content bb"><?=get_docName($row['Type']); ?></td>
								<td align="left" class="content bb"><a href="javascript:post_to_url('?page=e_doc&menu=dept_receive&sub=view_dept', {'ID':<?=$row['ID']?>, 'Seq':<?=$row['Seq']?>, 'Type':<?=$row['Type']?>});"><?=Br_iconv($row['Subject']); ?></a></td>
...
<?					} ?>
		...
		<!-- receive_wait_list PAGE NAVIGATION START -->
		<tr>
			<td align="center">
<?
...
?>
		...
		<!-- receive_wait_list PAGE NAVIGATION END -->

		<tr>
			<td height="30"></td>
		</tr>
		<!-- receive_wait_list LIST END -->
		<!-- receive_wait_list MAIN END -->
	</table>
</td>
<!-- receive_wait_list END -->
				</tr>
			</table>
		</td>	
	</tr>
