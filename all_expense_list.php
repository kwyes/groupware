...
if($mode == "search") {
	...
	$query = "SELECT ID, Type, Seq, Status, CompanyID, UserID, CONVERT(char(10), SubmitDate, 126) AS SubmitDate, Subject, RegDate ".
			 "FROM E_DOC_Header ".
			 "WHERE convert(char(10), SubmitDate, 126) <= '$today' AND Status = 1 AND Type = 3 ".
	...
	<table width="100%">
		...
		<tr>
			<td height="40">
				<table width="100%">
					<tr>
						<td width="360" align="left" class="content_title">지출결의서 List (회계팀)</td>
		...
		<form name="form_search" action="?page=e_doc&menu=expense_all" enctype="multipart/form-data" method="post" accept-charset="utf-8">
...
<?								$query = "SELECT docID, docName FROM DocKind WHERE docID = 3 ORDER BY docID";
