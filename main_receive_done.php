...
$query = "SELECT TOP 7 ID, Type, Seq, Status, CompanyID, UserID, CONVERT(char(10), SubmitDate, 126) AS SubmitDate, Subject, RegDate ".
		 "FROM E_DOC_Header ".
		 "WHERE convert(char(10), SubmitDate, 126) <= '$today' ".
...
<?					$is_appUserExisted = TRUE; ?>
					<tr height="25">
						<td class="docid bb"><a href="javascript:post_to_url('?page=e_doc&menu=receive&sub=view_done', {'ID':<?=$row['ID']?>, 'Seq':<?=$row['Seq']?>, 'Type':<?=$row['Type']?>});"><?=$ID; ?></a></td>
