...
$today = date("Y-m-d");
$Status = 3;	// 임시저장
$tempDate = date("Y-m-d", mktime(0, 0, 0, 12, 31, 2030));

if($mode == "search") {
	$docType = $_POST['search_docType'];
	$dateStart = $_POST['search_dateStart'];
	$dateEnd = (empty($_POST['search_dateEnd']) ? $tempDate : $_POST['search_dateEnd']);
...
<!-- offer_save_list START -->
<td width="" align="left" valign="top">
	<table width="100%">
		<!-- offer_save_list TITLE START -->
		<tr>
			<td height="40">
				<table width="100%">
					<tr>
						<td width="360" align="left" class="content_title">올린 결재 문서함 > 임시저장</td>
						<td align="right" style="padding: 14px;">&nbsp;</td> 
					</tr>
				</table>
			</td>
		</tr>
		<!-- offer_save_list TITLE END -->

		<!-- offer_save_list MAIN START -->
		<tr>
			<td height="1" valign="top" style="border-top: 1px #c9c9c9 solid"></td>
		</tr>

		<!-- offer_save_list SEARCH BOX START -->
		<form name="form_search" action="?page=e_doc&menu=offer&sub=save" enctype="multipart/form-data" method="post" accept-charset="utf-8">
				...
				<table width="100%" class="doc_border">
					<tr height="30">
						...
						<td width="200" style="padding: 3px 0 0 10px;">
							<select name="docType" style="width:187px;">
...
<?								} ?>
		...
		<!-- offer_save_list SEARCH BOX END -->

		<!-- offer_save_list LIST START -->
		<tr>
			<td>
...
?>
							<tr height="25">
								<td class="docid bb"><a href="javascript:post_to_url('?page=e_doc&menu=offer&sub=view_save', {'ID':'<?=$row['ID']?>', 'Seq':'<?=$row['Seq']?>', 'Type':'<?=$row['Type']?>', 'approval':3, 're_approval':2});"><?=$ID; ?></td>
								<td class="content bb"><?=get_docName($row['Type']); ?></a></td>
								<td class="content bb"><a href="javascript:post_to_url('?page=e_doc&menu=offer&sub=view_save', {'ID':'<?=$row['ID']?>', 'Seq':'<?=$row['Seq']?>', 'Type':'<?=$row['Type']?>', 'approval':3, 're_approval':2});"><?=Br_iconv($row['Subject']); ?></a></td>
								<td class="content bb"><?=get_doc_approval($row['Status']); ?></td>
								<td class="content bb"><?=($row['CompanyID'] != "" ? get_company_name($row['CompanyID']) : ""); ?></td>
...
<?					} ?>
		...
		<!-- offer_save_list PAGE NAVIGATION START -->
		<tr>
			<td align="center">
<?
...
?>
		...
		<!-- offer_save_list PAGE NAVIGATION END -->

		<tr>
			<td height="30"></td>
		</tr>
		<!-- offer_save_list LIST END -->
		<!-- offer_save_list MAIN END -->
	</table>
</td>
<!-- offer_save_list END -->
				</tr>
