...
<!-- offer_recovery_list START -->
<td width="" align="left" valign="top">
	<table width="100%">
		<!-- offer_recovery_list TITLE START -->
		<tr>
			<td height="40">
				<table width="100%">
					<tr>
						<td width="360" align="left" class="content_title">올린 결재 문서함 > 회수문서</td>
						<td align="right" style="padding: 14px;">&nbsp;</td> 
					</tr>
				</table>
			</td>
		</tr>
		<!-- offer_recovery_list TITLE END -->

		<!-- offer_recovery_list MAIN START -->
		<tr>
			<td height="1" valign="top" style="border-top: 1px #c9c9c9 solid"></td>
		</tr>

		<!-- offer_recovery_list SEARCH BOX START -->
		<form name="form_search" action="?page=e_doc&menu=offer&sub=recovery" enctype="multipart/form-data" method="post" accept-charset="utf-8">
...
<?								} ?>
		...
		<!-- offer_recovery_list SEARCH BOX END -->

		<!-- offer_recovery_list LIST START -->
		<tr>
			<td>
...
?>
							<tr height="25">
								<td class="docid bb"><a href="javascript:post_to_url('?page=e_doc&menu=offer&sub=view_recovery', {'ID':'<?=$row['ID']?>', 'Seq':'<?=$row['Seq']?>','Type':'<?=$row['Type']?>'});"><?=$ID; ?></a></td>
								<td class="content bb"><?=get_docName($row['Type']); ?></td>
								<td class="content bb"><a href="javascript:post_to_url('?page=e_doc&menu=offer&sub=view_recovery', {'ID':'<?=$row['ID']?>', 'Seq':'<?=$row['Seq']?>','Type':'<?=$row['Type']?>'});"><?=Br_iconv($row['Subject']); ?></a></td>
...
<?					} ?>
		...
		<!-- offer_recovery_list PAGE NAVIGATION START -->
		<tr>
			<td align="center">
<?
...
?>
		...
		<!-- offer_recovery_list PAGE NAVIGATION END -->

		<tr>
			<td height="30"></td>
		</tr>
		<!-- offer_recovery_list LIST END -->
		<!-- offer_recovery_list MAIN END -->
	</table>
</td>
<!-- offer_recovery_list END -->
				</tr>
			</table>
		</td>	
	</tr>
