...
$Get_next = "page=community&menu=note&cpage1=$cpage1&search_mode=$search_mode&search_keyword=$search_keyword";

$IT_where = " boardId = 4".$where_keyword;
...
$query = "SELECT TOP $IT_top bdId,boardId,CompanyId,UserId,bdEmail,dbPhone,bdTitle,bdDescription,bdHit,mainId,".
...
function go(){
	 ...
	 location.href="?page=community&menu=note&sub=write";
}
...
function del(id, type){
	...
	if(answer) {
		location.href="upload/upload_form.php?mode=note_delete&ID="+id+"&Type="+type;
	} else {
		return;
	}
}
			...
			<td style="padding:12px 25px">
				<table width="100%" class="doc_border">
					<tr height="33">
						...
						<input type="button" value="문서 올리기" onClick="javascript:go()"></td>
...
?>
							<tr height="25">
								<td align="center" class="docid bb"><a href="javascript:post_to_url('?page=community&menu=note&sub=view', {'ID':<?=$row['bdId']?>, 'Type':4,'cpage1':<?=$cpage1?>})"><?=$row['mainId']?></a></td>
								<td class="content bb"><a href="javascript:post_to_url('?page=community&menu=note&sub=view', {'ID':<?=$row['bdId']?>, 'Type':4,'cpage1':<?=$cpage1?>})"><?=Br_iconv($row['bdTitle'])?></a></td>
...
<?								if($UserID == $row['UserId']) {?>
								<a href="javascript:del('<?=$row['bdId']?>',4)"><font color="red">삭제</font></a>
<?								}?>
								</td>
							</tr>
<?							if($i++ == $page_per_list) {
