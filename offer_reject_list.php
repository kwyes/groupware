	...
	<table width="100%">
		...
		<tr>
			<td height="40">
				<table width="100%">
					<tr>
						<td width="360" align="left" class="content_title">올린 결재 문서함 > 반려뮨서</td>
		...
		<form name="form_search" action="?page=e_doc&menu=offer&sub=reject" enctype="multipart/form-data" method="post" accept-charset="utf-8">
...
?>
							<tr height="25">
								<td class="docid bb"><a href="javascript:post_to_url('?page=e_doc&menu=offer&sub=view_reject', {'ID':'<?=$row['ID']?>', 'Seq':'<?=$row['Seq']?>','Type':'<?=$row['Type']?>'});"><?=$ID; ?></a></td>
								<td class="content bb"><?=get_docName($row['Type']); ?></td>
								<td class="content bb"><a href="javascript:post_to_url('?page=e_doc&menu=offer&sub=view_reject', {'ID':'<?=$row['ID']?>', 'Seq':'<?=$row['Seq']?>','Type':'<?=$row['Type']?>'});"><?=Br_iconv($row['Subject']); ?></a></td>
								<td class="content bb" style="color:#DF0101;"><?=get_doc_approval($row['Status']); ?></td>
