	...
	<table width="100%">
		...
		<tr>
			<td height="40">
				<table width="100%">
					<tr>
						<td width="360" align="left" class="content_title">받은 결재 문서함 > 미결재 문서</td>
		...
		<form name="form_search" action="?page=e_doc&menu=receive&sub=wait" enctype="multipart/form-data" method="post" accept-charset="utf-8">
...
<?							$is_read = check_is_read($row['Type'], $row['ID'], $row['Seq'], $UserID); ?>
							<tr height="25" style="<?=($is_read == 0) ? "background-color:yellow" : ""; ?>">
								<td class="docid bb"><a href="javascript:post_to_url('?page=e_doc&menu=receive&sub=view_wait', {'ID':<?=$row['ID']?>, 'Seq':<?=$row['Seq']?>, 'Type':<?=$row['Type']?>});"><?=create_DocID($row['ID'], $row['Seq']); ?></a></td>
								<td class="content bb"><?=get_docName($row['Type']); ?></td>
								<td align="left" class="content bb"><?=Br_iconv($row['Subject']); ?></td>
...
<?					$i = 1; ?>
<?					while($row = mssql_fetch_array($query_result)) { ?>
<?						$AppUserCheck = check_ApprovalUser_wait($row['Type'], $row['ID'], $row['Seq']); ?>
<?						$is_read = check_is_read($row['Type'], $row['ID'], $row['Seq'], $UserID); ?>
